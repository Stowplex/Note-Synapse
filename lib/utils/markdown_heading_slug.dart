...
String slugifyHeading(String raw) {
  ...
  // Strip underscore emphasis delimiters without deleting underscores inside
  // ordinary identifiers such as `snake_case_id`.
  text = text.replaceAllMapped(
    RegExp(r'(?<![\p{L}\p{N}])_([^_\n]+)_(?![\p{L}\p{N}])', unicode: true),
    (m) => m.group(1) ?? '',
  );

  // Strip emphasis / code / strikethrough delimiters (order matters: longer
  // tokens first).
  for (final pattern in const ['**', '__', '~~', '*', '`']) {
