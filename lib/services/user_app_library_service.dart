...
class UserAppLibraryService {
  ...
  Future<UserAppLibrary> addLibrary({
    ...
    try {
      ...
      LoggerService.info(
        'Added library "$name" with ${dependencies.length} dependencies',
      );
  ...
  Future<List<UserAppLibrary>> getLibraries(
    String appUuid,
    int revisionId,
  ) async {
    try {
      final libraryMaps = await _databaseService.getUserAppLibraries(
        appUuid,
        revisionId,
      );
      return libraryMaps
  ...
  Future<List<UserAppLibraryDependency>> getDependencies(int libraryId) async {
    try {
      final dependencyMaps = await _databaseService
          .getUserAppLibraryDependencies(libraryId);
      return dependencyMaps
          .map(
            (map) => UserAppLibraryDependency(
              id: map['id'] as int,
  ...
  Future<void> copyLibrariesToRevision({
    ...
    try {
      LoggerService.info(
        'Copying libraries from revision $fromRevisionId to revision $toRevisionId for app $appUuid',
      );

      // Get all libraries from the source revision
      final sourceLibraries = await getLibraries(appUuid, fromRevisionId);
      LoggerService.debug(
        'Found ${sourceLibraries.length} libraries in source revision $fromRevisionId',
      );

      if (sourceLibraries.isEmpty) {
        LoggerService.warning(
          'No libraries found in source revision $fromRevisionId for app $appUuid',
        );
        return;
      }
      ...
      for (final library in sourceLibraries) {
        ...
        LoggerService.debug(
          'Copied library ${library.name} with ${dependencies.length} dependencies',
        );
      }

      LoggerService.info(
        'Successfully copied ${sourceLibraries.length} libraries to revision $toRevisionId',
      );
    } catch (e) {
