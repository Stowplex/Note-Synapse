...
class AppProvider extends ChangeNotifier {
  ...
  List<Note> getFilteredNotes(Filter filter, {List<Note>? base}) {
    // Folded ONCE here rather than per note: this runs on the widget build
    // path (notes_screen._filterNotes) and NFKC folding is several times the
    // cost of the `toLowerCase` it replaced.
    final text = filter.includeText;
    final foldedText = (text != null && text.isNotEmpty)
        ? foldForMatch(text)
        : null;
  ...
  ///
  /// [foldedText] is `filter.includeText` already folded with [foldForMatch];
  /// callers testing many notes against one filter pass it so the query is
  /// folded once instead of per note.
  static bool _matchesFilterCriteria(
    ...
    if (text != null && text.isNotEmpty) {
      // Substring match folded through NFKC (plan §1.6), so full-width and
      // compatibility characters compare equal the way the search index
      // normalizes them. The per-note folded text is memoized.
      if (!matchesFoldedQuery(note, foldedText ?? foldForMatch(text))) {
        return false;
      }
    }
