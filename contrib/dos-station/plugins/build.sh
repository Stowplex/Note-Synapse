...
description: "Play DOS games and run DOS files stored in your notes. Mounts a .zip attachment as the C: drive of a DOSBox/WebAssembly emulator, configured by a dosbox code block in the note. Code blocks tagged {dos-name=\"HELLO.BAS\"} and attachments you pick are copied onto C: and can be saved back. On-screen keyboard and gamepad overlays, and one-tap frame capture appended back to the note. Downloads the js-dos engine (~2 MB) on first run."
author: Note Synapse
