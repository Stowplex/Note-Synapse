...
void main() {
  ...
  group('ShareScreen', () {
    ...
    testWidgets('prefills the Space\'s tags as removable chips', (
      tester,
    ) async {
      await load();
  ...
  group('NoteCard', () {
    test('the active Space\'s own tags are dropped from the chips', () {
      ...
      expect(NoteCard.visibleTags(['thesis', '2026', 'urgent']), ['urgent']);
    });
    ...
    test('at most three chips, and no duplicate globe', () {
      ...
      expect(shown.where((t) => t == SpaceScopeService.allSpacesTag).length, 1);
    });
  });
