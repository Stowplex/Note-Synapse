...
void main() {
  ...
  group('replaceTag rewrites filters', () {
    test('renames the tag in includeTags, in the database and in memory',
        () async {
      await load(
    ...
    test('matches WHOLE TAGS ONLY: renaming thesis leaves thesis-2026 alone',
        () async {
    ...
    test('dedups when the filter already carries both the old and the new name',
        () async {
      await load(
        filters: [buildFilter('s1', includeTags: ['thesis', 'archive'])],
        tags: [buildTag('thesis'), buildTag('archive')],
      );
    ...
    test('a rename that puts a comma into a space tag retires the space',
        () async {
      await load(
        filters: [buildFilter('s1', includeTags: ['thesis'])],
    ...
    test('...and the retirement is what gets WRITTEN, so it survives a restart',
        () async {
    ...
    test('leaves filters that do not carry the tag untouched', () async {
      await load(
        filters: [buildFilter('f1', includeTags: ['other'], isSpace: false)],
        tags: [buildTag('thesis'), buildTag('archive')],
      );
  ...
  group('deleteTag rewrites filters', () {
    ...
    test('deleting ONE of several include tags leaves the space active',
        () async {
      await load(
        notes: [buildNote('n1', tags: ['thesis', 'reading'])],
        filters: [buildFilter('s1', includeTags: ['thesis', 'reading'])],
        tags: [buildTag('thesis')],
      );
      await provider.setActiveSpace('s1');
    ...
    test('deleting the space\'s ONLY include tag un-flags it, deactivates, and '
      ...
      await load(
        notes: [buildNote('n1', tags: ['thesis']), buildNote('n2')],
        filters: [buildFilter('s1', name: 'Thesis', includeTags: ['thesis'])],
        tags: [buildTag('thesis')],
      );
      await provider.setActiveSpace('s1');
      expect(provider.scopedNotes.map((n) => n.id), ['n1']);
    ...
    test('a plain filter emptied by the delete keeps its (false) isSpace flag',
        () async {
      await load(
  ...
  group('a tag containing a comma can never be a space tag', () {
    test('it is not listed as a space and cannot be activated', () async {
      ...
      await load(filters: [buildFilter('s1', includeTags: ['a,b'])]);
    ...
    test('one comma tag among several disqualifies the whole space', () async {
      await load(filters: [buildFilter('s1', includeTags: ['thesis', 'a,b'])]);
