...
void main() {
  ...
  setUp(() async {
    ...
    indexer = NoteIndexService(db, changeNotifier: notifier);
    scope = SpaceScopeService();
    getIt.registerSingleton<DatabaseService>(db);
    getIt.registerSingleton<SpaceScopeService>(scope);
    // The tool routes its ranked branch through SearchService, so the scope
    // rules are exercised where they now live: the NoteFilterContext the tool
    // builds, enforced during result accumulation.
    getIt.registerSingleton<SearchService>(
      SearchService(db, indexer, notesProvider: () => db.getAllNotes()),
    );
  ...
  group('composition, with notes on every side of the predicate', () {
    ...
    test(
      'with no tags of its own the Space alone decides, both branches',
      () async {
        const inScope = [
          'cross-invoice',
          'cross-only',
    ...
    test('scope: "all" drops the Space and keeps the caller\'s tags', () async {
      const everyInvoice = [
        'cross-invoice',
        'member-invoice',
        'outsider-invoice',
    ...
    test(
      'a requested tag the Space also requires still gates all-spaces',
      () async {
    ...
    test(
      'with no Space active nothing is scoped and nothing is ORed',
      () async {
