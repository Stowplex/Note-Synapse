...
Filter buildFilter(String id, {String? name, List<String> includeTags = const []}) {
  final now = DateTime(2026, 1, 1);
  return Filter(
    id: id,
    name: name ?? id,
...
void main() {
  ...
  /// What the tool handed to the FTS query.
  ///
  /// `tags` and `scopeTags` are separate on purpose: only the second is ORed
  /// with `all-spaces` (A7), so a test that could not tell them apart would
  /// pass with the two merged back into one list.
  ({List<String>? tags, List<String>? scopeTags, bool orAllSpaces})
    ...
    return (
      tags: call.captured[0] as List<String>?,
      scopeTags: call.captured[1] as List<String>?,
      orAllSpaces: call.captured[2] as bool,
    );
  }
  ...
  group('search_notes scoping', () {
    test('with no active Space the call is exactly what it always was',
        () async {
    ...
    test('inside a Space and with no tags, the Space\'s tags are the filter',
        () async {
      activateThesis();
    ...
    test('explicit tags COMPOSE with the Space rather than replacing it (A7)',
        () async {
      activateThesis();
    ...
    test('a tag the Space already requires still travels as the caller\'s',
        () async {
      activateThesis();
  ...
  group('the empty-query branch is scoped like the FTS branch', () {
    test('an empty query inside a Space filters on the Space\'s tags',
        () async {
      activateThesis();
      ...
      when(mockDb.getNotesByTag(SpaceScopeService.allSpacesTag))
          .thenAnswer((_) async => []);
    ...
    test('an all-spaces note is returned even without the Space\'s tags',
        () async {
      activateThesis();
      when(mockDb.getNotesByTag('thesis')).thenAnswer(
        (_) async => [buildNote('in', tags: ['thesis', '2026'])],
      );
    ...
    test('an all-spaces note that does carry them is returned', () async {
      ...
      when(mockDb.getNotesByTag('invoice')).thenAnswer(
        (_) async => [buildNote('in', tags: ['thesis', '2026', 'invoice'])],
      );
      when(mockDb.getNotesByTag(SpaceScopeService.allSpacesTag)).thenAnswer(
        (_) async => [
          buildNote('cross', tags: [SpaceScopeService.allSpacesTag]),
          buildNote('crossInvoice', tags: [
    ...
    test('a note that is both a member and all-spaces is returned once',
        () async {
      activateThesis();
      final both = buildNote('both', tags: [
        'thesis',
        '2026',
        SpaceScopeService.allSpacesTag,
      ]);
    ...
    test('scope: "all" with an empty query does not pull in all-spaces notes',
        () async {
      activateThesis();
      when(mockDb.getNotesByTag('cooking')).thenAnswer(
        (_) async => [buildNote('recipe', tags: ['cooking'])],
      );
  ...
  group('ls marks the active Space', () {
    ...
    test('nothing is marked when no Space is active', () async {
      when(mockDb.getAllFilters()).thenAnswer(
        (_) async => [buildFilter('s1', name: 'Thesis', includeTags: ['thesis'])],
      );
