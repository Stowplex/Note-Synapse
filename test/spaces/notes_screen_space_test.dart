...
/// Ranked search, faked as a case-insensitive title match over [notes].
///
/// Text search left the widget build path when the layered index landed: the
/// screen now intersects `SearchService`'s ranked ids with the browse list.
/// These tests are about the *composition* (scope → tab → chips → search), not
/// about ranking, so the cheapest thing that answers "which notes match" is
/// enough — but it has to be a real answer, not an empty one, or search would
/// look like it filtered everything out.
class _TitleMatchSearchService extends Fake implements SearchService {
  List<Note> notes = const [];
  int _seq = 0;
  SearchTicket? _newest;

  @override
  SearchTicket takeTicket() {
    _seq++;
    return _newest = _TestTicket(_seq);
  }

  @override
  bool isCurrent(SearchTicket ticket) =>
      ticket is! _TestTicket || _newest == ticket;
  ...
  SearchResponse _respond(String query, SearchTicket? ticket) {
  ...
  static const SearchResultChunk _chunk = SearchResultChunk(
    snippet: Snippet(
      text: '',
      matches: [],
...
/// [SearchTicket]'s constructor is private, so the fake mints its own.
class _TestTicket implements SearchTicket {
  const _TestTicket(this.seq);
...
/// The notes screen reads `NoteIndexService.progress` to decide whether to
/// show the "Building search index…" strip. Idle progress keeps the strip
/// hidden, which is all these Space tests want from it.
class _IdleIndexService extends Fake implements NoteIndexService {
...
void main() {
  ...
  late _TitleMatchSearchService fakeSearch;
  ...
  setUp(() async {
    ...
    fakeSearch = _TitleMatchSearchService();
    ...
    // NotesScreen builds a NotesSearchController in initState.
    getIt.registerSingleton<SearchService>(fakeSearch);
    getIt.registerSingleton<NoteIndexService>(_IdleIndexService());
  ...
  Future<void> load({
    ...
    // The ranked search runs over the same corpus; the screen intersects its
    // ids with the browse list, so scope/tab/chips still apply on top.
    fakeSearch.notes = notes;
  ...
  group('scoped list', () {
    ...
    testWidgets('a custom filter tab stays inside the Space (C2)', (
      ...
      final urgent = filter('urgent', name: 'Urgent', includeTags: ['urgent']);
      await load(
        notes: [
    ...
    testWidgets('with no active Space nothing is filtered out', (tester) async {
      await load(
        notes: [
          note('inside', tags: ['thesis']),
          note('outside', tags: ['recipes']),
  ...
  group('space chrome', () {
    testWidgets('the switcher shows the Space name and the Space\'s own tab '
      ...
      final other = filter('other', name: 'Recipes', includeTags: ['recipes']);
      await load(
        notes: [
          note('inside', tags: ['thesis']),
        ],
  ...
  group('activate as space (C4)', () {
    ...
    testWidgets('a filter carrying a reserved include tag cannot become a '
      ...
      expect(
        ...
        isFalse,
        reason:
            'a filter that can never be a Space must not be flagged as '
  ...
  group('hierarchy', () {
    ...
    testWidgets('with no active Space the dialog shows the whole tree', (
      ...
      expect(
        tester.widget<HierarchyDialog>(find.byType(HierarchyDialog)).rootFilter,
        isNull,
      );
    });
  });
