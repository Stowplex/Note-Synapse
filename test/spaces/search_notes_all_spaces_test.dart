...
/// `all-spaces` note.
Note buildNote(String id, {required String content, List<String> tags = const []}) {
...
void main() {
  ...
  test('an all-spaces note matches a scoped search only with the flag',
      () async {
    await seed([
      ...
      buildNote('outsider', content: 'quantum entanglement', tags: ['cooking']),
    ]);

    expect(
      await search('quantum entanglement', scopeTags: ['thesis']),
      ['member'],
    );
    expect(
      await search(
  ...
  test('the OR wraps the whole scope conjunction rather than adding another AND',
      () async {
    ...
    await seed([
      buildNote('member', content: 'quantum entanglement', tags: ['thesis', '2026']),
  ...
  test('a note carrying both the space tags and all-spaces appears once',
      () async {
    await seed([
      buildNote(
        'both',
  ...
  test('the caller\'s tags are ANDed outside the OR (A7)', () async {
    ...
    await seed([
      buildNote('member', content: 'quantum entanglement',
          tags: ['thesis', 'invoice']),
      buildNote('memberNoInvoice', content: 'quantum entanglement',
          tags: ['thesis']),
      buildNote('crossInvoice', content: 'quantum entanglement',
          tags: [SpaceScopeService.allSpacesTag, 'invoice']),
      buildNote('cross', content: 'quantum entanglement',
          tags: [SpaceScopeService.allSpacesTag]),
      buildNote('outsider', content: 'quantum entanglement',
          tags: ['invoice']),
    ]);
  ...
  test('the flag needs a scope: tags alone are never ORed away', () async {
    ...
    await seed([
      buildNote('member', content: 'quantum entanglement', tags: ['thesis']),
      buildNote('everywhere', content: 'quantum entanglement',
          tags: [SpaceScopeService.allSpacesTag]),
    ]);
  ...
  test('the flag changes nothing when no tags are given', () async {
    ...
    expect(
      await search('quantum entanglement'),
      ['everywhere', 'plain'],
    );
    expect(
      await search('quantum entanglement', includeAllSpacesTag: true),
      ['everywhere', 'plain'],
    );
  });

  test('the default leaves an ordinary tagged search exactly as it was',
      () async {
    await seed([
  ...
  test('the flag matches nothing extra when no note carries all-spaces',
      () async {
    await seed([
      buildNote('member', content: 'quantum entanglement', tags: ['thesis']),
      buildNote('outsider', content: 'quantum entanglement', tags: ['cooking']),
