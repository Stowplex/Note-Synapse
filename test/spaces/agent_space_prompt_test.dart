...
void main() {
  ...
  Future<String> executorPrompt() async {
    ...
    ).thenReturn(ContextNode(id: 'node1', objective: 'desc'));
    when(
      mockAIService.generateWithAttachments(
        any,
        any,
  ...
  group('the chat prompt', () {
    ...
    test(
      'every gated name is a real tool name, so a rename cannot silence it',
      () {
    ...
    test(
      'the chat screen delegates to it rather than re-implementing the gate',
      () {
