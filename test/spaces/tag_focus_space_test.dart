...
void main() {
  ...
  group('focusOnTag', () {
    test(
      'creates a one-tag Space named after the tag and activates it',
      () async {
        await load(
          notes: [
            note('n1', tags: ['thesis']),
    ...
    test(
      'focusing the Space that is already active is still a reuse',
      () async {
        await load();
        final first = await focusOnTag(provider, 'thesis');
        final second = await focusOnTag(provider, 'thesis');
    ...
    test('reuses a Space that already existed before any focus', () async {
      await load(
        filters: [
          filter('s1', name: 'Thesis', includeTags: ['thesis']),
        ],
      );
    ...
    test('does not reuse a Space that scopes to more than this tag', () async {
      await load(
        filters: [
          filter('s1', includeTags: ['thesis', '2026']),
        ],
      );
    ...
    test('does not adopt a plain filter carrying the same tag', () async {
      ...
      await load(
        filters: [
          filter('f1', includeTags: ['thesis'], isSpace: false),
        ],
      );
    ...
    test(
      'reports failure and activates nothing when the write fails',
      () async {
        await load();
    ...
    test(
      'a reserved tag is refused even when a Space already carries it',
      () async {
  ...
  group('the tag chip affordance', () {
    ...
    testWidgets('the reserved cross-Space tag explains why it cannot be a '
      ...
      await load(
        notes: [
          note('n1', tags: [SpaceScopeService.allSpacesTag]),
        ],
        tags: [
          Tag(
            id: 't1',
