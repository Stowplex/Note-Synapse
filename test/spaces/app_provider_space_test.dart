...
void main() {
  ...
  group('setActiveSpace', () {
    ...
    test('rejects a filter that is not flagged isSpace', () async {
      await load(filters: [buildFilter('f1', includeTags: ['x'], isSpace: false)]);
    ...
    for (final reserved in [
      ...
      test('rejects a space whose include tags contain "$reserved"', () async {
        ...
        expect(
          scope.stampTags,
          isEmpty,
          reason: 'stamping all-spaces onto every note created in the Space '
              'is exactly what A2 forbids',
        );
      });
    }

    test('a restored backup row flagged isSpace with a reserved tag is inert',
        () async {
      ...
      await load(
        filters: [
          buildFilter('restored', includeTags: [
            SpaceScopeService.allSpacesTag,
          ]),
        ],
      );
    ...
    test('rejects an unknown id and leaves the current space alone', () async {
      await load(filters: [buildFilter('s1', includeTags: ['thesis'])]);
      await provider.setActiveSpace('s1');
  ...
  group('activation lifecycle', () {
    ...
    test('a persisted space survives a restart', () async {
      ...
      when(mockDb.getAllNotes()).thenAnswer(
        (_) async => [buildNote('in', tags: ['thesis']), buildNote('out')],
      );
    ...
    test('deleting the active space deactivates it and clears prefs',
        () async {
      final space = buildFilter('s1', includeTags: ['thesis']);
      await load(filters: [space]);
      await provider.setActiveSpace('s1');
    ...
    test('editing the include tags re-points the stamp', () async {
      ...
      await provider.updateFilter(space.copyWith(includeTags: ['thesis', '2026']));
    ...
    test('a preferences failure does not break loadData (A15)', () async {
      ...
      await load(
        notes: [buildNote('n1', tags: ['thesis'])],
        filters: [buildFilter('s1', includeTags: ['thesis'])],
      );
  ...
  group('scopedNotes', () {
    test('with no active space it holds every note, in the same order (A3)',
        () async {
    ...
    test('is unmodifiable in both branches, so a mutating caller fails at the '
      ...
      await load(
        notes: [buildNote('in', tags: ['thesis']), buildNote('out')],
        filters: [buildFilter('s1', includeTags: ['thesis'])],
      );

      expect(() => provider.scopedNotes.add(buildNote('x')),
    ...
    test('keeps only notes carrying every include tag, in notes order',
        () async {
      await load(
        notes: [
        ...
        filters: [buildFilter('s1', includeTags: ['thesis', '2026'])],
      );
      await provider.setActiveSpace('s1');
  ...
  group('scope narrows lists, never access (invariant 4)', () {
    test('an out-of-space note stays reachable by id and by saved filter',
        () async {
      final byId = buildFilter('ideas', includeTags: ['idea'], isSpace: false);
      await load(
        notes: [
          buildNote('inSpace', tags: ['thesis', 'idea']),
          buildNote('outside', tags: ['idea']),
        ],
        filters: [buildFilter('s1', includeTags: ['thesis']), byId],
      );
      await provider.setActiveSpace('s1');
      ...
      expect(
        provider.scopedNotes.where((n) => n.id == 'outside'),
        isEmpty,
      );

      // ...and nothing else does. `notes` still resolves it by id,
      expect(
        provider.notes.where((n) => n.id == 'outside').single.tags,
        ['idea'],
      );
      // and an unscoped getFilteredNotes still finds it.
      expect(
        provider.getFilteredNotes(byId).map((n) => n.id).toSet(),
        {'inSpace', 'outside'},
      );
    });
  });
  ...
  group('membership predicates', () {
    test('noteInScope is the wider tags-only approximation of _isInSpace',
        () async {
      await load(
        notes: [buildNote('n1', tags: ['thesis', 'draft'])],
        filters: [
  ...
  group('scopedNotes cache invalidation (C1)', () {
    Future<void> loadThesisSpace() async {
      await load(
        notes: [buildNote('n1'), buildNote('n2', tags: ['thesis'])],
        filters: [buildFilter('s1', includeTags: ['thesis'])],
    ...
    test('deleteTag refreshes the scope', () async {
      ...
      await load(
        ...
        filters: [buildFilter('s1', includeTags: ['thesis', 'reading'])],
  ...
  group('addNote stamping', () {
    Future<void> activateThesis() async {
      await load(filters: [buildFilter('s1', includeTags: ['thesis', '2026'])]);
      await provider.setActiveSpace('s1');
    }

    test('stamps by default, before the insert, as an order-preserving union',
        () async {
      ...
      final inserted = verify(mockDb.insertNote(captureAny)).captured.single
          as Note;
    ...
    test('fromShare and applySpaceTags are orthogonal', () async {
      ...
      var inserted = verify(mockDb.insertNote(captureAny)).captured.single
          as Note;
  ...
  group('updateNote never stamps', () {
    test('an existing note keeps exactly the tags it was given', () async {
      ...
      final updated = verify(mockDb.updateNote(captureAny)).captured.single
          as Note;
  ...
  group('createNewNotes stamping', () {
    test('stamps every generated note before persisting it', () async {
      ...
      final inserted = verify(mockDb.insertNote(captureAny)).captured
          .cast<Note>();
  ...
  group('joinSpace / leaveSpace', () {
    ...
    test('both reject an unknown space and leave every note alone', () async {
      await load(
        notes: [buildNote('n1', tags: ['thesis'])],
        filters: [buildFilter('f1', includeTags: ['thesis'], isSpace: false)],
      );
    ...
    test('leaveSpace removes everything when no other space claims the tags',
        () async {
      await load(
        notes: [buildNote('n1', tags: ['thesis', '2026', 'idea'])],
        filters: [buildFilter('thesis', includeTags: ['thesis', '2026'])],
      );
    ...
    test('a plain filter with an overlapping tag does not protect it (J6)',
        () async {
      ...
      await load(
        notes: [buildNote('n1', tags: ['thesis', '2026'])],
        filters: [
          buildFilter('thesis', includeTags: ['thesis', '2026']),
    ...
    test('success is read from the write, not from the shared error field',
        () async {
    ...
    test('leaveSpace ignores a space the note does not actually belong to',
        () async {
      // Reading requires {reading, 2026}; the note has reading but not 2026,
      // so it is not a member and cannot protect 2026.
      await load(
        notes: [buildNote('n1', tags: ['thesis', '2026', 'reading'])],
        filters: [
          buildFilter('thesis', includeTags: ['thesis', '2026']),
    ...
    test('leaveSpace never removes all-spaces', () async {
      await load(
        ...
        filters: [buildFilter('thesis', includeTags: ['thesis'])],
      );
    ...
    test('a filter that includes a reserved tag is not a Space to leave',
        () async {
    ...
    test('leaveSpace computes the removal set per note', () async {
      ...
      final updated = verify(mockDb.updateNote(captureAny)).captured
          .cast<Note>();
      expect(
    ...
    test('leaveSpace is a no-op for a note that is not in the space',
        () async {
      await load(
        notes: [buildNote('n1', tags: ['cooking'])],
        filters: [buildFilter('s1', includeTags: ['thesis'])],
      );
  ...
  group('date lists are scoped', () {
    test('getTasksForDate and getNotesForDate honour the space', () async {
      ...
      expect(
        provider.getTasksForDate(DateTime(2026, 3, 4)).map((n) => n.id),
        ['inTask', 'outTask'],
      );
      expect(
        provider.getNotesForDate(DateTime(2026, 3, 4)).map((n) => n.id),
        ['inTask', 'outTask'],
      );
  ...
  group('getFilteredNotes base (C2)', () {
    ...
    Future<void> loadWithCustomFilter() async {
      ...
      await load(
        ...
        filters: [buildFilter('s1', includeTags: ['thesis']), customFilter],
      );
      await provider.setActiveSpace('s1');
    }
    ...
    test('without a base it still sees every note (the default is _notes)',
        () async {
      await loadWithCustomFilter();
