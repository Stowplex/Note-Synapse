...
void main() {
  ...
  group('allSpaces: true escapes the scope', () {
    test('lists every skill inside a Space', () async {
      ...
      expect(index.keys, containsAll(<String>[
        'skill-filed',
        'skill-everywhere',
        'skill-unfiled',
      ]));
    ...
    group('through the legacy noteId argument', () {
      ...
      test('load_skill refuses a skill filed under another Space', () async {
        ...
        expect(
          (result as Map)['error'],
          contains('not found'),
          reason: 'the same answer as a missing note: which Spaces a skill is '
              'filed under is not something a hidden skill should disclose',
        );
      });

      test('load_skill refuses a Space-filed skill with no Space active (A4)',
          () async {
        inNoSpace();
      ...
      test('load_skill still serves a visible skill by noteId', () async {
        ...
        expect(
          await tool.execute({'noteId': 'skill-filed'}),
          isA<String>(),
        );
        expect(
      ...
      test('an unfiled skill is reachable only with no Space active',
          () async {
        inNoSpace();
      ...
      test('the agent pins by noteId only after the same visibility test', () {
        ...
        expect(
          ...
          reason: 'the noteId branch returns before the skillRef branch, so '
  ...
  group('constructor shape', () {
    test('the positional-only form still compiles and defaults to shared',
        () async {
    ...
    test('an omitted scope resolves the shared one, not a private one',
        () async {
  ...
  group('only the management surfaces escape the scope', () {
    ...
    test('no other lib/ file passes allSpaces: true', () {
      ...
      expect(offenders, isEmpty, reason: 'unexpected allSpaces: true call site');
    });
