...
void main() {
  ...
  final thesis = space(
    'thesis',
    name: 'Thesis',
    includeTags: ['thesis', '2026'],
  );
  final reading = space(
    'reading',
    name: 'Reading',
    includeTags: ['reading', '2026'],
  );
  ...
  group('join', () {
    ...
    testWidgets('joining reports how many the Space still hides', (
      ...
      final tasksOnly = space(
        'tasks',
        name: 'Sprint',
        includeTags: ['sprint'],
        noteTypes: [NoteType.task],
      );
      await load(
        notes: [
          note('a-note'),
  ...
  group('leave', () {
    testWidgets('unchecking removes only the tags no other Space needs', (
      ...
      await load(
        notes: [
          note('n1', tags: ['thesis', '2026', 'reading']),
        ],
        filters: [thesis, reading],
      );
      await openPicker(tester, ['n1']);
  ...
  group('show in every space', () {
    testWidgets('adds and removes the all-spaces tag', (tester) async {
      ...
      expect(await setShowInEverySpace(provider, ['n1'], true), isTrue);
      expect(tagsOf('n1'), contains(SpaceScopeService.allSpacesTag));

      expect(await setShowInEverySpace(provider, ['n1'], false), isTrue);
