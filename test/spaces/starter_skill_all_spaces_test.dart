...
void main() {
  ...
  group('starter skills carry all-spaces', () {
    ...
    test('an installed starter skill is visible from inside a Space', () async {
      await StarterService.installStarterSkills();
      expect(inserted, isNotEmpty);
      when(
        mockDb.getNotesByTag(SkillService.agentSkillTag),
      ...
      scope.setActive(spaceS.id, spaceS.includeTags);
      final index = await SkillService(
        mockDb,
        spaceScope: scope,
      ).buildSkillIndex();
  ...
  group('correction C5: install stays idempotent with a Space active', () {
    ...
    test('an unfiled skill with the same ref still blocks re-install inside a '
        'Space', () async {
      ...
      when(
        mockDb.getNotesByTag(SkillService.agentSkillTag),
      ).thenAnswer((_) async => [existingSkill('older-install', bundledRef)]);
      scope.setActive(spaceS.id, spaceS.includeTags);

      // Scoped to the one ref this test seeds. More than one starter skill
      // ships now, and the others are genuinely not installed here — the
      // claim under test is about the seeded ref, not the bundle size.
      final count = await StarterService.installStarterSkills(
        skillRefs: {bundledRef},
      );
      expect(count, 0);
      expect(inserted, isEmpty);
    });
