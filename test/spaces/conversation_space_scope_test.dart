...
void main() {
  ...
  group('createConversation(tags:)', () {
    ...
    test(
      'tags nothing by default, so every existing caller is unchanged',
      () async {
  ...
  group('getAllConversations(includeAllSpacesTag:)', () {
    test(
      'without the flag the tags are ANDed and all-spaces is invisible',
      () async {
    ...
    test(
      'with the flag an all-spaces conversation joins the Space\'s own',
      () async {
        await seed('member', tags: ['thesis', '2026']);
        await seed('partial', tags: ['thesis']);
        await seed('everywhere', tags: [SpaceScopeService.allSpacesTag]);
    ...
    test(
      'the tags stay ANDed inside the OR: a partial match is still out',
      () async {
        await seed('partial', tags: ['thesis']);
        await seed('everywhere', tags: [SpaceScopeService.allSpacesTag]);
    ...
    test(
      'the same all-spaces conversation appears inside EVERY Space',
      () async {
    ...
    test('the other filters still apply on the OR path', () async {
      ...
      expect(await scoped(['thesis'], conversationIds: ['member']), ['member']);
      // maxAge keeps both (they were just written).
      expect(await scoped(['thesis'], maxAge: const Duration(days: 3650)), [
        'everywhere',
        'member',
      ]);
    });
    ...
    test(
      'includeEmpty: false drops message-less conversations on the OR path',
      () async {
        await seed('everywhere', tags: [SpaceScopeService.allSpacesTag]);
        await seed('member', tags: ['thesis']);
  ...
  group('scopeTags compose with the caller\'s own tags', () {
    ...
    Future<void> seedComposition() async {
      ...
      await seed(
        'urgentEverywhere',
        tags: ['urgent', SpaceScopeService.allSpacesTag],
      );
    ...
    test(
      'a cross-Space conversation still has to match the caller\'s tags',
      () async {
        await seedComposition();
    ...
    test('the flag fails closed: no scope tags, no OR', () async {
      ...
      expect(await query(['thesis'], includeAllSpacesTag: true), ['member']);
    });
    ...
    test('scope tags without the flag are a plain extra conjunction', () async {
      ...
      expect(await query(['urgent'], scopeTags: ['thesis']), ['member']);
    });
  });
  ...
  group('the conversation tree', () {
    test(
      'seeded with a Space\'s tags still shows the all-spaces conversation',
      () async {
        await seed('member', tags: ['thesis']);
        await seed('everywhere', tags: [SpaceScopeService.allSpacesTag]);
        await seed('outsider', tags: ['cooking']);
