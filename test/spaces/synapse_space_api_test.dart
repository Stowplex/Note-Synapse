...
@GenerateNiceMocks([MockSpec<AppProvider>(), MockSpec<InAppWebViewController>()])
void main() {
  ...
  group('Synapse.space in the bootstrap script', () {
    ...
    test(
      'a Space name containing a quote, a backslash and a newline stays '
      'valid JSON',
      () {
        ...
        final literal = spaceLiteral(
          buildBridge().buildBootstrapScript().source,
        );
        expect(
          literal,
          contains(r'\"'),
  ...
  group('prompt documentation (CLAUDE.md: implement AND document)', () {
    ...
    test('the cross-Space OR is documented around the Space tag group only',
        () async {
