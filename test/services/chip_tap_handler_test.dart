...
void main() {
  ...
  test('handle() forks with chip.label as suggested title, then addUserMessage with chip.prompt, then onSendUserPrompt', () async {
  ...
  test('handle() short-circuits if fork returns null — no addUserMessage, no send', () async {
  ...
  test('handle() rethrows if forkFromMessageInContext throws (unexpected error)', () async {
    const chip = ChipAction(label: 'l', prompt: 'p');
    when(mockFork.forkFromMessageInContext(
      forkFromMessageId: anyNamed('forkFromMessageId'),
      sourceConversationId: anyNamed('sourceConversationId'),
