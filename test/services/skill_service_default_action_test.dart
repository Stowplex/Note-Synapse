...
class _EmptyDb extends Fake implements DatabaseService {
  @override
  Future<List<Note>> getNotesByTag(String tag) async => const [];
}
...
''';
  ...
  test(
    'existing fields (name, description, enabled, skill_ref) still parse correctly with default_action present',
    () {
      const content = '''---
name: Mixed Skill
...
''';
  ...
  test(
    'REGRESSION: parser still ignores skills with no name or no description',
    () {
      const noName = '''---
description: D
---
...
''';
  ...
  test(
    'buildDefaultActionPromptSection returns empty string when no skills declare default_action',
    () {
  ...
  test(
    'buildDefaultActionPromptSection returns empty string for empty index',
    () {
  ...
  test(
    'buildDefaultActionPromptSection concatenates default_action strings ordered by skillRef',
    () {
      final index = <String, SkillMetadata>{
      ...
      expect(
        ...
        isFalse,
        reason:
            'Skills without defaultAction should be omitted entirely from the section',
      );
  ...
  test(
    'buildDefaultActionPromptSection includes a section header so the AI knows the role of the text',
    () {
      final index = <String, SkillMetadata>{
  ...
  test(
    'buildDefaultActionPromptSection excludes disabled skills (defense in depth)',
    () {
  ...
  test(
    'buildDefaultActionPromptSection treats whitespace-only defaultAction as null',
    () {
      final index = <String, SkillMetadata>{
  ...
  test(
    'bundled Knowledge Exploration starter skill advertises chip emission',
    () {
      final content = File(
        'assets/starter/skills/Knowledge_Exploration.md',
      ).readAsStringSync();
      final meta = svc.parseSkillMetadata('starter-skill', content);
      expect(meta, isNotNull);
      expect(meta!.name, 'Knowledge Exploration');
      expect(meta.skillRef, 'knowledge-exploration');
      expect(meta.defaultAction, contains('```chips'));
      expect(meta.defaultAction, contains('## use analogy'));
      expect(meta.defaultAction, contains('## go deeper'));

      final section = svc.buildDefaultActionPromptSection({meta.noteId: meta});
      expect(section, contains('Knowledge Exploration'));
      expect(section, contains('```chips'));
    },
  );

  test(
    'buildSkillIndex can include bundled Knowledge Exploration skill',
    () async {
      final withBundled = SkillService(_EmptyDb(), includeBundledSkills: true);

      final index = await withBundled.buildSkillIndex();
      ...
      expect(
        withBundled.buildDefaultActionPromptSection(index),
        contains('```chips'),
      );
    },
  );
