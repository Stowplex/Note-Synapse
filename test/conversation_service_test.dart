  ...
  group('Conversation Service Tests', () {
    ...
    group('Basic CRUD Operations', () {
      test('Create conversation and add messages', () async {
        ...
        expect(conversationWithMessages.messages[0].type, equals(MessageType.user));
        expect(conversationWithMessages.messages[1].type, equals(MessageType.ai));
      });
      ...
      test('Build conversation tree', () async {
        ...
        expect(tree!.nodes.length, greaterThan(1)); // At least root + 1 interaction
      });
      ...
      test('Delete conversation', () async {
        ...
        final deletedConversation = await conversationService.getConversationWithMessages(conversation.id);
        expect(deletedConversation, isNull);
      });
    });

    group('Forking Operations', () {
      test('Fork conversation creates new conversation with messages up to fork point', () async {
        ...
        final forkedMessages = await conversationService.getConversationWithMessages(forkedConversation.id);
        expect(forkedMessages, isNotNull);
        expect(forkedMessages!.messages.length, equals(2)); // Up to and including fork point
      });

      test('Add interaction to forked conversation creates new branch in tree', () async {
        final originalConversation = await conversationService.createConversation(
          title: 'Original',
        );
        ...
        final forkPointNode = tree!.nodes.values
            .firstWhere((node) => node.messageId == aiResponse.id);
        expect(forkPointNode.children.length, greaterThan(0));
      });

      test('Fork from forked conversation without throwing exception', () async {
        final originalConversation = await conversationService.createConversation(
          title: 'Original Conversation',
        );
        ...
        final secondForkedMessages = await conversationService.getConversationWithMessages(secondForkedConversation.id);
        expect(secondForkedMessages!.messages.length, equals(2));
        expect(secondForkedMessages.messages[0].content, equals('Question A'));
      ...
      test('Fork from forked conversation with new messages', () async {
        ...
        final secondForkMessages = await conversationService.getConversationWithMessages(secondFork!.id);
        expect(secondForkMessages!.messages.length, equals(4));
        expect(secondForkMessages.messages[0].content, equals('Original question'));
        expect(secondForkMessages.messages[1].content, equals('Original answer'));
        expect(secondForkMessages.messages[2].content, equals('Forked question'));
      ...
      test('Multiple forks from same point', () async {
        ...
        final fork1Messages = await conversationService.getConversationWithMessages(fork1.id);
        final fork2Messages = await conversationService.getConversationWithMessages(fork2.id);
        
        expect(fork1Messages!.messages.length, equals(fork2Messages!.messages.length));
      });

      test('Fork preserves note associations', () async {
        final note = await databaseService.insertNote(Note(
          id: 'test-note-id',
          title: 'Test Note',
    ...
    group('Message Deletion', () {
      test('Delete single message with proper cleanup', () async {
        ...
        final aiMessages = conversationWithMessages.messages.where((m) => m.type == MessageType.ai).toList();
        final firstAIMessage = aiMessages.first;
        
        await conversationService.deleteMessageWithSubtree(firstAIMessage.id);
        
        final updatedConversation = await conversationService.getConversationWithMessages(conversation.id);
        expect(updatedConversation, isNotNull);
        expect(updatedConversation!.messages.length, equals(1)); // Only the first user message should remain
      });
      ...
      test('Delete message with subtree', () async {
        ...
        final remainingConversation = await conversationService.getConversationWithMessages(conversation.id);
        expect(remainingConversation, isNotNull);
        expect(remainingConversation!.messages.length, equals(1)); // Only user message remains
        
        // The fork should be deleted since its messages are part of the subtree
        final deletedFork = await conversationService.getConversationWithMessages(fork1.id);
      ...
      test('Garbage collect empty conversations after deletion', () async {
        ...
        final messages = await conversationService.getConversationWithMessages(conversation.id);
      ...
      test('Delete message with attachments', () async {
        ...
        await databaseService.insertConversationAttachment(ConversationAttachment(
          id: 'attachment-1',
        ...
        final attachments = await databaseService.getConversationAttachments(userMessage.id);
      ...
      test('Tree structure remains intact after partial deletion', () async {
        ...
        final originalMessages = await conversationService.getConversationWithMessages(conversation.id);
        expect(originalMessages!.messages.length, equals(3)); // Q1, A1, Q2
        
        // Fork should be intact with Q1, A1, Q3, A3
        final forkMessages = await conversationService.getConversationWithMessages(fork.id);
    ...
    group('Note Validation', () {
      test('Validate conversation notes', () async {
        ...
        final invalidNotes = await conversationService.validateConversationNotes(conversation.id);
      ...
      test('Cleanup invalid note references', () async {
        ...
        final updatedConversation = await databaseService.getConversation(conversation.id);
