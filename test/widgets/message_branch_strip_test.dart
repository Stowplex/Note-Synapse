...
void main() {
  testWidgets('renders nothing when only one branch (single-child fork-points are not shown)', (tester) async {
    await tester.pumpWidget(_wrap(MessageBranchStrip(
  ...
  testWidgets('renders one row per branch and highlights the active one with a key', (tester) async {
    await tester.pumpWidget(_wrap(MessageBranchStrip(
      branches: [
        _summary('a', 'Branch A'),
        _summary('b', 'Branch B'),
  ...
  testWidgets('fan-out >5: shows first 5 + "+N more" tap to expand inline', (tester) async {
    await tester.pumpWidget(_wrap(MessageBranchStrip(
  ...
  testWidgets('same-document sibling tap fires onSwitchBranch with confirmedDocumentSwap=false', (tester) async {
    String? switched;
    bool? wasConfirmed;
    await tester.pumpWidget(_wrap(MessageBranchStrip(
      branches: [
  ...
  testWidgets('different-document sibling tap shows confirm dialog before firing switch', (tester) async {
    String? switched;
    bool? wasConfirmed;
    await tester.pumpWidget(_wrap(MessageBranchStrip(
      branches: [
    ...
    expect(switched, isNull, reason: 'Switch should not fire until user confirms');
    // Tap Switch button.
  ...
  testWidgets('different-document tap canceled: switch never fires', (tester) async {
    String? switched;
    await tester.pumpWidget(_wrap(MessageBranchStrip(
      branches: [
        _summary('a', 'A', notes: const ['note-X']),
