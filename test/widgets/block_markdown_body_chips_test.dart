...
void main() {
  testWidgets(
    ...
    await tester.pumpWidget(_wrap(BlockMarkdownBody(
      content: '''Hello world.
...
```''',
  ...
  testWidgets(
      ...
      (tester) async {
    int notifyCount = 0;
    Widget build(String content) => _wrap(BlockMarkdownBody(
  ...
  testWidgets('notifies callback again when content changes', (tester) async {
    ...
    await tester.pumpWidget(build('''New data.
```chips
## new chip
New prompt body.
```'''));
  ...
  testWidgets('works without onChipsExtracted callback (optional)',
    ...
    await tester.pumpWidget(_wrap(const BlockMarkdownBody(
      content: '''Reply.
```chips
## L
P.
