...
void main() {
  group('Conversation Feature Tests', () {
    ...
    group('Tree Node Creation Tests', () {
      test(
        '1. Create conversation without messages - no node created',
        () async {
      ...
      test(
        '5. Fork from A with interaction - Node C created as child of A',
        () async {
      ...
      test('6. Complex tree with multiple forks', () async {
        ...
        final node1 = tree.nodes.values.firstWhere(
          (n) => n.messageId == ai1.id,
        );
        final node2 = tree.nodes.values.firstWhere(
          (n) => n.messageId == ai2.id,
        );
        final node3 = tree.nodes.values.firstWhere(
          (n) => n.messageId == ai3.id,
        );
        final node4 = tree.nodes.values.firstWhere(
          (n) => n.messageId == ai4.id,
        );
    ...
    group('Basic Conversation Operations', () {
      ...
      test('should get conversation with messages', () async {
        ...
        expect(
          conversationWithMessages.messages[0].content,
          equals('First message'),
        );
        expect(
          conversationWithMessages.messages[1].content,
          equals('First AI response'),
        );
        expect(
          conversationWithMessages.messages[2].content,
